"""Command-line interface for pgcarter (Typer).
...
from pgcarter.analyzer.runner import AnalysisInputError, run_analysis
...
def index(
    ...
    try:
        ...
        log.error("pgcarter index failed: %s", exc)
...
def analyze(
    ...
    try:
        ...
        log.error("pgcarter analyze failed: %s", exc)
        log.debug("Traceback:", exc_info=True)
        raise typer.Exit(1) from exc
...
def main(argv: list[str] | None = None) -> None:
    """Entry point for the ``pgcarter`` console script."""
    app(args=argv)
